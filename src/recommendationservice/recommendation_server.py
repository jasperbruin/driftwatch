    ...
    def ListRecommendations(self, request, context):
        ...
        with tracer.start_span("ml-model"):
            ...
            if not list(request.product_ids):
                ...
                input_product_id = list(request.product_ids)[0]
                logger.debug(f"Received request for product_id: {input_product_id}")
                ids = find_similar_products(input_product_id, X,
                                            max_responses)
                logger.debug(f"Recommended product_ids: {ids}")
        ...
        signal.set_current_value(
            hellinger_stat_test(ratings["rating"], ratings_test["rating"],
...
def create_matrix(df):
    ...
    logger.debug(f"Number of unique users: {N}, Number of unique products: {M}")
    ...
    logger.debug(f"Created sparse matrix X with shape {X.shape}")
...
def find_similar_products(product_id, X, k, show_distance=False):
    ...
    logger.debug(f"Product index for product_id {product_id}: {product_ind}")

    product_vec = X[product_ind]
    product_vec = product_vec.reshape(1, -1)
    logger.debug(f"Product vector shape: {product_vec.shape}")

    neighbour = kNN.kneighbors(product_vec, return_distance=show_distance)
    logger.debug(f"Nearest neighbours indices: {neighbour}")
    ...
    logger.debug(f"Neighbour product IDs: {neighbour_ids}")
...
if __name__ == "__main__":
    ...
    # Start Prometheus client HTTP server on port 9464
    logger.info("Starting Prometheus client HTTP server on port 9464")
