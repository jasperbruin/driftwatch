...
func (fe *frontendServer) productHandler(w http.ResponseWriter, r *http.Request) {
	...
	//MV: get recommendation header
	is_recommendation := r.Header.Get("recommendation")
	//MV: check if current request comes from a recommendation, and message erb-detector service if so
	if is_recommendation == "true" {
		resp, err_rec := http.Get("http://erbdetector:5000/?value=0")
		if err_rec != nil {
			renderHTTPError(log, r, w, errors.Wrap(err_rec, "recommendation status not sent"), http.StatusInternalServerError)
		} else if resp != nil{
...
func (fe *frontendServer) addToCartHandler(w http.ResponseWriter, r *http.Request) {
	...
	//MV: recommendation not taken, send value to erb-detector
	resp, err_rec := http.Get("http://erbdetector:5000/?value=0")
	if err_rec != nil {
		renderHTTPError(log, r, w, errors.Wrap(err_rec, "recommendation status not sent"), http.StatusInternalServerError)
	} else if resp != nil{
