...
def browseProduct(l):
    ...
    ctr_prediction = model.predict({"user_id": pd.Series([random.randrange(100)]), "parent_asin": pd.Series([product_mapper[recommendations[0]]]),"year": pd.Series([random.randrange(10)]), "month": pd.Series([random.randrange(12)]),"day": pd.Series([random.randrange(28)])})
