...
def _summarise(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (df
            ...
            .assign(
                speedup_vs_full = lambda d: d.loc[d.method == "no_pca", "mean_time_taken"].iloc[0] / d["mean_time_taken"],
                memory_reduction = lambda d: d.loc[d.method == "no_pca", "mean_memory"].iloc[0] / d["mean_memory"]
            )
            .rename(columns={
                "method": "Method",
...
def _plot_tradeoffs(df, output_dir):
    ...
    ax.set_title("Tradeoff Between Metric X and Metric Y", fontsize=16, fontweight="bold")
...
def main():
    ...
    ap.add_argument("--results_path", required=True, help="Path to results.json produced by the experiment script")
    ap.add_argument("--out_dir", default="figs", help="Directory to store CSV and PNG outputs")
    args = ap.parse_args()
