...
def test_valid_init(valid_k):
    ...
    """
    transformer = KLLTransformer(k=valid_k)
    assert transformer.k == valid_k, f"KLLTransformer should store the correct value of k={valid_k}."
...
def test_fit_valid_data():
    ...
    transformer.fit(X)
    assert transformer.kll_sketches is not None, "kll_sketches should be initialized after fit."
    assert len(transformer.kll_sketches) == 3, "There should be one KLL sketch per dimension."
...
def test_transform_basic():
    ...
    transformer = KLLTransformer(k=30)
